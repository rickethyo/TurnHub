...
| Atlas touchscreen calibration | Atlas display (`atlas_display.cpp`) | `atlas-touch/cal2` blob (2026-09-25; the 2026-09-24 `cal` key held values solved from mis-sampled readings, is ignored, and is removed on the next save): raw `TouchCalibration` struct (swap flag, raw range per screen axis). Missing or invalid falls back to `config.h` and triggers on-device calibration at boot. Device presentation, not game state |

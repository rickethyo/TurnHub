...
U1 is the removable, complete 38-pin ESP32 DevKit carrier interface (Inland ESP32-WROOM-32E, micro-USB). Its custom symbol and local symbol library use **A1–A19 and J1–J19 as pin numbers**, not ESP32 module pad numbers. Its footprint, `Sigil.pretty/ESP32_DevKit_38_Socket_Row22.86mm` (from `build_schematic.py`), has 2.54 mm pitch and rows 22.86 mm (0.9 in) apart, and uses the published 55.0 x 27.5 mm outline. Inland publishes 25.4 mm rows, but the owner's DevKit straddles a standard breadboard with one free hole outside each row (columns b and i), which is 22.86 mm. A 7 mm keep-out (no tracks, vias, pours or parts; an estimate) covers the WROOM-32E antenna at the A19/J19 end. It is drawn from the carrier's top side with the DevKit face up and micro-USB at the top, which mirrors the rear photo: the A row is on the left, the J row on the right, and A1/J1 at the USB end. *Needs verification*: caliper-check the row spacing and test-fit sockets before ordering. The PCB files are empty placeholders.

...
class LEDController:
    def __init__(self, serial_controller: SerialController) -> None:
        ...
        self.cache: dict[tuple[str, int], int | bool] = {}

    def clear_cache(self) -> None:
        self.cache.clear()

    def _set_cached(self, kind: str, module: int, value: int | bool) -> None:
        key = (kind, module)
        ...
        if kind == "BLUE":
            self.serial.blue(module, int(value))
        elif kind == "RED":
            self.serial.red(module, bool(value))
        elif kind == "GREEN":
            self.serial.green(module, bool(value))

    def blue(self, module: int, brightness: int) -> None:
        self._set_cached("BLUE", module, max(0, min(255, int(brightness))))
    ...
    def breathe_value(now: float) -> int:
        phase = (now % BREATHE_PERIOD_SECONDS) / BREATHE_PERIOD_SECONDS
        sine = (math.sin(phase * 2.0 * math.pi - math.pi / 2.0) + 1.0) / 2.0

        # Slightly shape the curve so the breathing feels less linear.
        shaped = sine ** 2.2
        return int(BREATHE_MIN + shaped * (BREATHE_MAX - BREATHE_MIN))

    @staticmethod
    def player_number_red_on(player_number: int, now: float) -> bool:
        flash_block = LOBBY_FLASH_ON + LOBBY_FLASH_OFF
        sequence_length = player_number * flash_block + LOBBY_FLASH_GAP
    ...
    def render_lobby(self, lobby: Lobby, now: float | None = None) -> None:
        if now is None:
            now = time.monotonic()

        for module in lobby.module_ids:
            if lobby.is_joined(module):
                player_number = lobby.player_number(module)

                self.blue(module, 255 if module == lobby.selected_starter else 0)
                self.green(module, module == lobby.host_module)
                self.red(
                    module,
                    self.player_number_red_on(player_number, now)
    ...
    def render_game(
        ...
        if game.state == STATE_GAME_OVER:
            self.render_game_over(game, host_module)
            return
        ...
        for module in game.players:
            ...
            if turn_elapsed < NEW_TURN_FLASH_SECONDS:
                blink_on = int(turn_elapsed / NEW_TURN_FLASH_INTERVAL) % 2 == 0
            ...
            if phase == "NORMAL":
                ...
                self.green(module, False)
                blink_on = int(now / WARNING_BLINK_INTERVAL) % 2 == 0
                self.red(module, blink_on)

    def render_game_over(self, game: GameEngine, host_module: int | None) -> None:
        for module in game.players:
            self.blue(module, 255 if module == game.winner_module else 0)
            self.red(module, False)

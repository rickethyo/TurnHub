...
let authenticated=false,joined=false,state='LOBBY';
const api=http.createServer(async(req,res)=>{
 ...
 switch(url.pathname){
  ...
  case '/api/seats':result={seats:joined?[{module:8,slot:1,slotName:'A',player:1,name:'Phone Tester',profileId:'AB12CD34',virtual:true,hasPin:true}]:[]};break;
  ...
  case '/api/session/me':if(!authenticated){res.statusCode=401;break}result={authenticated:true,participating:joined,host:joined,virtual:true,name:'Phone Tester',profileId:'AB12CD34',hasPin:true,module:joined?8:255,slot:1,player:joined?1:0,active:state==='RUNNING'};break;
  case '/api/session/join':joined=true;result={ok:true,message:'Joined'};break;
  case '/api/control/start':state='RUNNING';result={ok:true};break;
...
(async()=>{
 ...
 try{
  ...
  await tab.getByRole('button',{name:'Create profile',exact:true}).click();
  ...
  console.log('PASS portal smoke: profile creation, join/start controls, mobile fit, wider desktop, no JS errors');

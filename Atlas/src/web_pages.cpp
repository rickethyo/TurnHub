...
<section id="adminSetup" class="notice good" hidden><strong>Set up this Atlas</strong><p>Create or sign into your account in My Account. Then, on the Atlas screen, hold Unlock admin for 3 seconds and select the button below within a minute to establish the initial Admin.</p><button class="primary" onclick="setupAdmin()">Make my account the initial Admin</button></section>
...
<div class="grid">
 <section class="card half"><div class="card-head"><h2 class="eyebrow">Atlas Wi-Fi security</h2></div>
  ...
  <div id="networkRestartNotice" class="notice" style="margin-top:14px">Unlock admin on the Atlas screen (hold Unlock admin for 3 seconds), then save within a minute. Changing the password restarts Atlas and disconnects every Wi-Fi client until it reconnects with the new password.</div>
 </section>
 <section class="card half"><div class="card-head"><h2 class="eyebrow">Atlas</h2></div>
  <div class="status-row"><span>State</span><strong id="stateMetricSystem">—</strong></div><div class="status-row"><span>Firmware</span><strong id="firmwareMetric">—</strong></div><div class="status-row"><span>Build</span><strong id="buildMetric">—</strong></div><div class="status-row"><span>ESP-NOW</span><strong id="espMetric">—</strong></div><div class="status-row"><span>Admin unlock</span><strong id="adminUnlockMetric">Locked</strong></div><div class="status-row"><span>Atlas OTA</span><strong id="otaMetric">Locked</strong></div><div class="status-row"><span>Active player</span><strong id="activeMetric">None</strong></div><div class="status-row"><span>Winner</span><strong id="winnerMetric">None</strong></div>
 ...
 <section class="card half"><div class="card-head"><div><h2 class="eyebrow">Paired Sigils</h2><p class="small">Unlock admin on the Atlas screen before renaming a device. Forget removes a Sigil's pairing; pair it again with Pair a Sigil on the Atlas screen and the Sigil's Pair button. Forget works in the lobby, for Sigils nobody is seated on. Holding a Sigil's own Pair button for 10 seconds makes the Sigil forget Atlas.</p></div></div><div id="atlasDevice"></div><div id="deviceSettingsList" class="stack" style="margin-top:10px;gap:10px"></div>
  ...
  <p class="hint">Return table to lobby ends a match in progress as a draw (every player gets a draw in their statistics) and clears the table, even if nobody can reach their Sigil. Factory reset erases everything a device has saved and restarts it: for a Sigil its pairing and settings; for Atlas every profile, PIN, pairing, Wi-Fi password and setting (the microSD card is kept). Unlock admin on the Atlas screen first.</p>
...
async function renameSigil(id){const d=deviceById(id);if(!d)return;const next=await uiAsk({title:'Rename device',text:'Unlock admin on the Atlas screen first (hold Unlock admin for 3 seconds).',ok:'Save name',input:{label:`Custom name for ${d.defaultLabel||('Sigil '+(Number(id)+1))}`,value:d.customName||'',maxLength:32}});if(next===null)return;await saveSigilName(id,next.trim())}
...
async function factoryResetSigil(id){const d=deviceById(id);if(!d)return;if(!await uiAsk({title:'Factory reset this Sigil?',text:`“${d.label}” erases everything it has saved, including its pairing, and restarts. Atlas forgets it; pair it again to use it. Unlock admin on the Atlas screen first (hold Unlock admin for 3 seconds).`,ok:'Factory reset',danger:true}))return;await factoryResetRequest(`module=${Number(id)}`)}
async function factoryResetAtlas(){const typed=await uiAsk({title:'Factory reset Atlas?',text:'Atlas erases every profile, PIN, statistic it holds, Sigil pairing, the Wi-Fi password and all settings, then restarts as new. The microSD card is not erased. This cannot be undone. Unlock admin on the Atlas screen first (hold Unlock admin for 3 seconds).',ok:'Factory reset Atlas',danger:true,input:{label:'Type RESET to confirm',value:'',maxLength:5}});if(typed===null)return;if(String(typed).trim().toUpperCase()!=='RESET'){showToast('Not reset: type RESET to confirm.',true);return}await factoryResetRequest('atlas=1',true)}
async function factoryResetRequest(query,atlas){try{const r=await fetch('/api/device/factory-reset?'+query,{method:'POST',headers:authHeaders()});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not factory reset');showToast(d.message||'Factory reset started.');if(atlas){resetTableStatus.textContent='Atlas is restarting as new. Rejoin its Wi-Fi with the default password, then set it up again.';return}await refreshAll()}catch(e){showToast(e.message,true)}}

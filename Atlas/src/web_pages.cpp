...
<div class="grid">
 ...
 <section class="card half"><div class="card-head"><div><h2 class="eyebrow">Paired Sigils</h2><p class="small">Hold the Atlas master button while renaming a device. Forget removes a Sigil's pairing; pair it again with Pair a Sigil on the Atlas screen and the Sigil's Pair button. Forget works in the lobby, for Sigils nobody is seated on. Holding a Sigil's own Pair button for 10 seconds makes the Sigil forget Atlas.</p></div></div><div id="atlasDevice"></div><div id="deviceSettingsList" class="stack" style="margin-top:10px;gap:10px"></div>
  <div class="actions" style="margin-top:10px"><button class="warn" onclick="forgetAllSigils()">Forget all Sigils</button></div>
  ...
  <p id="pairingWindowStatus" class="msg" role="status" aria-live="polite"></p></section>
...
function refreshAll(){
 ...
 refreshInFlight=(async()=>{
  try{
   ...
   const pairing=n?await pollJson('/api/pairing',authHeaders()):null;
   ...
   renderDevices(d);renderDeviceSettings(d);if(n)renderNetwork(n);if(pairing)renderPairing(pairing);renderStatus(s);renderGameLife();renderCounterControls();renderAccess();

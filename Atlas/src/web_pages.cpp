...
<div class="grid">
 ...
 <section class="card half"><div class="card-head"><div><h2 class="eyebrow">Paired Sigils</h2><p class="small">Renaming, Return table to lobby and Factory reset ask for the code the Atlas screen shows (Verify at the table). Forget removes a Sigil's pairing; pair it again with Pair a Sigil on the Atlas screen and the Sigil's Pair button. Forget works in the lobby, for Sigils nobody is seated on. Holding a Sigil's own Pair button for 10 seconds makes the Sigil forget Atlas.</p></div></div><div id="atlasDevice"></div><div id="deviceSettingsList" class="stack" style="margin-top:10px;gap:10px"></div>
  ...
  <label for="pairingWindowSelect">Atlas pairing window</label><select id="pairingWindowSelect" aria-describedby="pairingWindowHelp" onchange="savePairingWindow()"></select>
  ...
  <label for="speakerVolumeSelect">Atlas speaker volume</label><select id="speakerVolumeSelect" aria-describedby="speakerVolumeHelp" onchange="saveSpeakerVolume()"><option value="0">Off</option><option value="1">Low</option><option value="2">Medium (default)</option><option value="3">High</option></select>
  ...
  <p id="speakerVolumeStatus" class="msg" role="status" aria-live="polite"></p></section>
...
function renderPairing(p){const sel=pairingWindowSelect;if(document.activeElement===sel)return;const html=(p.choicesMs||[]).map(ms=>`<option value="${Number(ms)}">${Number(ms)/1000} seconds${Number(ms)===Number(p.sigilWindowMs)?' (default)':''}</option>`).join('');if(sel.innerHTML!==html)sel.innerHTML=html;sel.value=String(p.windowMs)}
function renderSpeaker(v){const sel=speakerVolumeSelect;if(document.activeElement===sel)return;sel.value=String(v.volume)}
async function saveSpeakerVolume(){const sel=speakerVolumeSelect,label=sel.options[sel.selectedIndex].text;try{const r=await fetch('/api/speaker?volume='+Number(sel.value),{method:'POST',headers:authHeaders()});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not save the speaker volume');speakerVolumeStatus.textContent=`Atlas speaker volume: ${label}.`;showToast('Speaker volume saved.')}catch(e){speakerVolumeStatus.textContent=e.message;showToast(e.message,true);await refreshAll()}}
async function savePairingWindow(){const ms=Number(pairingWindowSelect.value);try{const r=await fetch('/api/pairing?windowMs='+ms,{method:'POST',headers:authHeaders()});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not save the pairing window');pairingWindowStatus.textContent=`Atlas pairing window: ${ms/1000} seconds.`;showToast('Pairing window saved.')}catch(e){pairingWindowStatus.textContent=e.message;showToast(e.message,true);await refreshAll()}}

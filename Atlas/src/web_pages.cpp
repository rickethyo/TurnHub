...
<div class="grid">
 ...
 <section class="card half"><div class="card-head"><div><h2 class="eyebrow">Paired Sigils</h2><p class="small">Hold the Atlas master button while renaming a device. Forget removes a Sigil's pairing; pair it again with the Pair buttons on Atlas and the Sigil. Forget works in the lobby, for Sigils nobody is seated on. Holding a Sigil's own Pair button for 10 seconds makes the Sigil forget Atlas.</p></div></div><div id="atlasDevice"></div><div id="deviceSettingsList" class="stack" style="margin-top:10px;gap:10px"></div>
  ...
  <p id="pairingWindowHelp" class="hint">How long Atlas listens after its Pair button is pressed. A Sigil listens for 15 seconds, so with a longer window press Pair on Atlas first, then on the Sigil.</p>
...
async function forgetSigil(id){const d=deviceById(id);if(!d)return;if(!await uiAsk({title:'Forget this Sigil?',text:`Atlas forgets “${d.label}”. It must be paired again with the Pair buttons on Atlas and the Sigil before anyone can use it.`,ok:'Forget Sigil',danger:true}))return;await forgetRequest(`module=${Number(id)}`)}

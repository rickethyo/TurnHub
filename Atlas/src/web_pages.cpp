...
<section id="adminSetup" class="notice good" hidden><strong>Set up this Atlas</strong><p>Create or sign into your account in My Account, then select the button below. The Atlas screen shows a six-digit code; enter it here (or scan the QR code on the Atlas screen with this phone) to prove you are at the table.</p><button class="primary" onclick="setupAdmin()">Make my account the initial Admin</button></section>
...
<div class="col">
 ...
 <section class="card table-card">
  ...
  <div class="metrics"><div class="metric"><label>Players</label><strong id="playersMetric">0</strong></div><div class="metric"><label>Sigils</label><strong id="sigilsMetric">0</strong></div><div class="metric"><label>Starter</label><strong id="starterMetric">None</strong></div></div>
...
<div class="grid">
 ...
 <section class="card half"><div class="card-head"><div><h2 class="eyebrow">Appearance and feedback</h2><p class="small">Saved in this browser only. No sign-in needed.</p></div></div>
  <fieldset><legend class="sr-only">Portal theme</legend><p class="small" style="margin-top:0">Themes change only how this browser looks. They never change the game.</p>
   <div class="themes">
    <label class="theme-opt"><input type="radio" name="theme" value="brass"><strong>Brass</strong><span class="swatch" aria-hidden="true"><i style="background:#110d09"></i><i style="background:#30251a"></i><i style="background:#e2ae4a"></i><i style="background:#62d6b2"></i></span><small>Steampunk gold, rivets and gauges.</small></label>
    <label class="theme-opt"><input type="radio" name="theme" value="midnight"><strong>Midnight</strong><span class="swatch" aria-hidden="true"><i style="background:#0a0d14"></i><i style="background:#202a40"></i><i style="background:#e8b44f"></i><i style="background:#5ad8ca"></i></span><small>Clean modern dark with gold.</small></label>
    <label class="theme-opt"><input type="radio" name="theme" value="parchment"><strong>Parchment</strong><span class="swatch" aria-hidden="true"><i style="background:#ece2cd"></i><i style="background:#fbf6ea"></i><i style="background:#8e5f0c"></i><i style="background:#0d7560"></i></span><small>Light sepia for bright rooms.</small></label>
    <label class="theme-opt"><input type="radio" name="theme" value="contrast"><strong>High contrast</strong><span class="swatch" aria-hidden="true"><i style="background:#000"></i><i style="background:#fff"></i><i style="background:#ffd400"></i><i style="background:#00f0d8"></i></span><small>Maximum legibility. Chosen automatically when your device asks for more contrast.</small></label>
   </div></fieldset>
  <div class="switch-row" style="margin-top:14px"><div><label for="reduceMotionToggle">Reduce motion</label><small id="reduceMotionHelp">Stops the turning gear, gauge sweeps and other animation in this browser. Your device's reduced-motion setting also does this.</small></div><input id="reduceMotionToggle" type="checkbox" aria-describedby="reduceMotionHelp"></div>
  <h3 style="margin:22px 0 4px">Browser feedback</h3>
  <div class="switch-row"><div><label for="soundToggle">Browser sound</label><small id="soundToggleHelp">Play lightweight feedback tones on this device.</small></div><input id="soundToggle" type="checkbox" aria-describedby="soundToggleHelp"></div>
  <div class="switch-row"><div><label for="vibrationToggle">Vibration</label><small id="vibrationToggleHelp">Use haptics when supported by the browser.</small></div><input id="vibrationToggle" type="checkbox" aria-describedby="vibrationToggleHelp"></div>
  <div class="inline" style="margin-top:6px;align-items:end"><div class="field"><label for="volumeSelect" style="margin-top:8px">Feedback volume</label><select id="volumeSelect"><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select></div><button class="blue" onclick="playFeedback('test',true)">Test feedback</button></div>
  <div class="actions" style="margin-top:14px"><button class="primary" onclick="saveAppearance()">Save appearance</button></div>
  <p id="appearanceStatus" class="msg" role="status" aria-live="polite"></p>
 </section>
 ...
 <section class="card half"><div class="card-head"><div><h2 class="eyebrow">Personalization</h2><p class="small">How you show up at the table. Saved with your profile on Atlas's microSD card.</p></div></div>
  ...
  <div id="jewelBox" hidden><fieldset><legend>Avatar</legend>
   ...
   <label class="inline" style="gap:6px;align-items:center;margin:0"><input id="jewelStandard" type="checkbox"> Use the standard colors instead</label></div>
   ...
   <div class="actions" style="margin-top:12px"><button class="primary" type="button" onclick="savePersonalization()">Save personalization</button></div>
...
<div class="grid">
 <section class="card half"><div class="card-head"><h2 class="eyebrow">Atlas Wi-Fi security</h2></div>
  ...
  <div id="networkRestartNotice" class="notice" style="margin-top:14px">Saving asks for the code the Atlas screen shows, to prove you are at the table. Changing the password restarts Atlas and disconnects every Wi-Fi client until it reconnects with the new password.</div>
 </section>
 <section class="card half"><div class="card-head"><h2 class="eyebrow">Atlas</h2></div>
  <div class="status-row"><span>State</span><strong id="stateMetricSystem">—</strong></div><div class="status-row"><span>Firmware</span><strong id="firmwareMetric">—</strong></div><div class="status-row"><span>Build</span><strong id="buildMetric">—</strong></div><div class="status-row"><span>ESP-NOW</span><strong id="espMetric">—</strong></div><div class="status-row"><span>Verified at the table</span><strong id="adminUnlockMetric">No one</strong></div><div class="status-row"><span>Atlas OTA</span><strong id="otaMetric">Locked</strong></div><div class="status-row"><span>Active player</span><strong id="activeMetric">None</strong></div><div class="status-row"><span>Winner</span><strong id="winnerMetric">None</strong></div>
 ...
 <section class="card half"><div class="card-head"><div><h2 class="eyebrow">Paired Sigils</h2><p class="small">Renaming, Return table to lobby and Factory reset ask for the code the Atlas screen shows (Verify at the table). Forget removes a Sigil's pairing; pair it again with Pair a Sigil on the Atlas screen and the Sigil's Pair button. Forget works in the lobby, for Sigils nobody is seated on. Holding a Sigil's own Pair button for 10 seconds makes the Sigil forget Atlas.</p></div></div><div id="atlasDevice"></div><div id="deviceSettingsList" class="stack" style="margin-top:10px;gap:10px"></div>
  <div class="actions" style="margin-top:10px"><button onclick="verifyAtTable()">Verify at the table</button><button onclick="stopVerification()">Stop</button><span id="presenceStatus" class="small" role="status" aria-live="polite"></span></div>
  <div class="actions" style="margin-top:10px"><button class="warn" onclick="forgetAllSigils()">Forget all Sigils</button><button class="warn" onclick="resetTableToLobby()">Return table to lobby</button><button class="warn" onclick="factoryResetAtlas()">Factory reset Atlas</button></div>
  <p class="hint">Return table to lobby ends a match in progress as a draw (every player gets a draw in their statistics) and clears the table, even if nobody can reach their Sigil. Factory reset erases everything a device has saved and restarts it: for a Sigil its pairing and settings; for Atlas every profile, PIN, pairing, Wi-Fi password and setting (the microSD card is kept). Once verified at the table, an Admin stays verified for 10 minutes.</p>
  <p id="resetTableStatus" class="msg" role="status" aria-live="polite"></p>
  <label for="pairingWindowSelect">Atlas pairing window</label><select id="pairingWindowSelect" aria-describedby="pairingWindowHelp" onchange="atlasHardwareDirty=true"></select>
  <p id="pairingWindowHelp" class="hint">How long Atlas listens after Pair a Sigil is tapped on its screen. A Sigil listens for 15 seconds, so with a longer window tap Pair on Atlas first, then press Pair on the Sigil.</p>
  <p id="pairingWindowStatus" class="msg" role="status" aria-live="polite"></p>
  <label for="speakerVolumeSelect">Atlas speaker volume</label><select id="speakerVolumeSelect" aria-describedby="speakerVolumeHelp" onchange="atlasHardwareDirty=true"><option value="0">Off</option><option value="1">Low</option><option value="2">Medium (default)</option><option value="3">High</option></select>
  <p id="speakerVolumeHelp" class="hint">Atlas's own speaker plays table-wide cues: the start countdown, turn changes, turn-timer warnings, pause and resume, eliminations, wins and game over, so players without a Sigil hear them too. Every cue also shows on screen. Sigil sound is set per player in their accessibility settings.</p>
  <p id="speakerVolumeStatus" class="msg" role="status" aria-live="polite"></p>
  <div class="actions" style="margin-top:12px"><button class="primary" onclick="saveAtlasHardware()">Save Atlas settings</button></div></section>
...
<script>
let refreshInFlight=null,gameSettingsData=null,gameSettingsDirty=false,atlasHardwareDirty=false,gameSettingsSaving=false,lifeBusy=false,lifeStep=1,lifeBigStep=5;
...
let avatarIcons=null,myAvatar=0;
async function loadAvatarIcons(){try{const r=await fetch('/api/avatars',{cache:'force-cache'});const d=await r.json();if(r.ok)avatarIcons=d.avatars||[]}catch(e){}}
loadAvatarIcons();
// One preset as an inline SVG, drawn from the same 16x16 rows Atlas and the Sigils use.
function avatarSvg(id){const a=avatarIcons&&avatarIcons[Number(id)-1];if(!a)return '';let d='';a.rows.forEach((row,y)=>{let x=0;while(x<row.length){if(row[x]==='#'){let e=x;while(e<row.length&&row[e]==='#')e++;d+=`M${x} ${y}h${e-x}v1h-${e-x}z`;x=e}else x++}});return `<svg viewBox="0 0 16 16" width="70%" height="70%" shape-rendering="crispEdges" aria-hidden="true"><path d="${d}" fill="currentColor"/></svg>`}
function avatar(name,n,cls='',icon=0){const svg=avatarSvg(icon);return `<span class="avatar ${cls}" style="--hue:${hue(n)}" aria-hidden="true">${svg||initials(name)}</span>`}
...
function setReduceMotion(on){try{if(on)localStorage.setItem('turnhubReduceMotion','1');else localStorage.removeItem('turnhubReduceMotion')}catch(_){}loadMotionPref()}
...
function renderAvatarGrid(current){const opts=[{id:0,label:'None'}].concat((avatarIcons||[]).map(a=>({id:a.id,label:a.label})));avatarGrid.innerHTML=opts.map(o=>`<label class="theme-opt" style="text-align:center"><input type="radio" name="avatarPick" value="${o.id}" ${Number(current)===o.id?'checked':''}><span class="avatar" style="--hue:${hue(1)};margin:4px auto">${o.id?avatarSvg(o.id):'&ndash;'}</span><small>${esc(o.label)}</small></label>`).join('')}
async function savePersonalization(){const a=document.querySelector('input[name=avatarPick]:checked');const fields={avatar:a?a.value:'0',color:jewelStandard.checked?'none':jewelColor.value};try{const r=await fetch('/api/session/personalization',{method:'POST',headers:{...authHeaders(),'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams(fields)});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not save');myAvatar=Number(d.avatar)||0;renderAvatarGrid(d.avatar);jewelStandard.checked=!d.color;jewelStatus.textContent='Saved. Your Sigil and the table update within a few seconds.';showToast('Personalization saved.')}catch(e){jewelStatus.textContent=e.message;showToast(e.message,true)}}
async function loadJewel(){if(!avatarIcons)await loadAvatarIcons();try{const r=await fetch('/api/session/personalization',{headers:authHeaders(),cache:'no-store'});const d=await r.json();if(!r.ok)throw new Error(d.error||'Unavailable');jewelBox.hidden=false;personalSignedOut.hidden=true;myAvatar=Number(d.avatar)||0;renderAvatarGrid(d.avatar);if(d.color)jewelColor.value=d.color;jewelStandard.checked=!d.color;jewelStatus.textContent=!d.card?'Insert a microSD card in Atlas to choose a color.':d.color?'Current color: '+d.color:'Using the standard colors.'}catch(e){jewelBox.hidden=true;personalSignedOut.hidden=false}}
...
function saveBrowserPrefs(){browserPrefs={sound:soundToggle.checked,vibration:vibrationToggle.checked,volume:volumeSelect.value};try{localStorage.setItem('turnhubBrowserPrefs',JSON.stringify(browserPrefs))}catch(_){}}
function saveAppearance(){const t=document.querySelector('input[name=theme]:checked');setTheme(t?t.value:'brass');setReduceMotion(reduceMotionToggle.checked);saveBrowserPrefs();appearanceStatus.textContent='Saved in this browser.';showToast('Appearance saved.')}
...
function winnerText(s){return s.state==='GAME_OVER'&&!Number(s.winner)?'Draw (no winner)':playerLabel(s.winner)}
function renderStatus(s){statusData=s;dot.className='dot online';connPill.className='conn ok';connection.textContent='Connected';connectionMetric.textContent='Connected';playersMetric.textContent=s.players;sigilsMetric.textContent=s.sigils;starterMetric.textContent=playerLabel(s.starter);stateMetric.textContent=s.state;stateMetricSystem.textContent=s.state;firmwareMetric.textContent='v'+s.firmware;buildMetric.textContent=s.build||'—';espMetric.textContent=s.espNow?'Ready':'Error';activeMetric.textContent=playerLabel(s.active);activeMetricGame.textContent=playerLabel(s.active);winnerMetric.textContent=winnerText(s);adminUnlockMetric.textContent=s.presenceCodeShown?'Code on the Atlas screen':(s.presenceActive?'An Admin is verified':'No one');otaMetric.textContent=s.otaStateAllowed?'State ready':'Game locked';starterGame.textContent=playerLabel(s.starter);confirmGame.textContent=playerLabel(s.winConfirm);eliminationGame.textContent=playerLabel(s.eliminationTarget);winnerGame.textContent=winnerText(s);
 let title='Ready',sub='Sign in and join from your phone, or press Action on a Sigil',eye='Lobby',bs='';
 if(s.state==='LOBBY'){title=s.players?`${s.players} player${Number(s.players)===1?'':'s'} seated`:'Ready';sub=s.starter?`${playerLabel(s.starter)} goes first`:'Sign in and join from your phone, or press Action on a Sigil';if(s.starter)bs+=badge(`${playerLabel(s.starter)} starter`,'blue')}
 ...
 else if(s.state==='GAME_OVER'){eye='Game over';title=s.winner?playerLabel(s.winner):'Draw';sub=s.winner?'Winner confirmed':'Ended as a draw at Atlas; no winner';bs+=badge(s.winner?'Game over':'Draw','good')}
...
function renderPlayers(){const cards=seatData.seats.map(s=>{let cls='player';if(s.active)cls+=' active';if(statusData&&Number(statusData.winner)===Number(s.player))cls+=' winner';if(s.eliminated)cls+=' eliminated';const current=sameSessionSeat(s),name=s.name||`Player ${s.player}`;let controls='';if(s.virtual&&!current)controls=`<a class="btn small" href="/login?profile=${encodeURIComponent(s.profileId)}">Sign into this profile</a>`;else if(current)controls='<button class="small" disabled>Current browser seat</button>';else{controls+=`<button class="small" onclick="claimSeat(${s.module},${s.slot})">Use this seat</button>`;if(s.hasPin&&!s.virtual)controls+=`<button class="small blue" onclick="pinLogin(${s.module},${s.slot})">PIN login</button>`}let flags='';if(s.active)flags+=badge('Active','active');if(s.sessionClaimed)flags+=badge('Browser linked','good');if(s.hasPin)flags+=badge('PIN');if(s.eliminated)flags+=badge('Out','bad');return `<article class="${cls}"><div class="player-head">${avatar(name,s.player,'',s.avatar)}<div><div class="player-name">${esc(name)}</div><div class="small">Player ${s.player} · ${esc(sigilLabel(s.module))} · Seat ${esc(s.slotName)}</div></div><span class="player-state ${s.active?'on':''}">${esc(playerStateText(s))}</span></div>${flags?`<div class="badges">${flags}</div>`:''}<div class="actions">${controls}</div></article>`});const html=cards.length?cards.join(''):'<div class="empty">No players have joined yet.</div>';if(players.innerHTML!==html)players.innerHTML=html}
function seatsForModule(id){return seatData.seats.filter(s=>Number(s.module)===Number(id))}
function capabilityHtml(x){const caps=[];if(Number(x.capabilities)&1)caps.push(badge('Display','good'));if(x.display==='oled')caps.push(badge('OLED: 1 player','blue'));else if(x.display==='epaper')caps.push(badge('E-paper: up to 2 players','blue'));if(!x.metadata)caps.push(badge('Legacy metadata','warn'));if(x.customName)caps.push(badge('Custom name','blue'));return caps.join('')}
function renderDevices(d){renderPresence();
...
function renderSession(){
 ...
 for(const a of [sessionAvatar,profileAvatar,accountAvatar]){a.innerHTML=avatarSvg(myAvatar)||initials(name);a.style.setProperty('--hue',hue(joined?me.player:0))}
 ...
 const stateHtml=(me.active?badge('Your turn','active'):'')+(me.eliminated?badge('Eliminated','bad'):'');
...
async function presenceState(){try{const r=await fetch('/api/presence',{headers:authHeaders(),cache:'no-store'});return r.ok?await r.json():null}catch(_){return null}}
async function confirmPresence(code){try{const r=await fetch('/api/presence/confirm?code='+encodeURIComponent(code),{method:'POST',headers:authHeaders()});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not verify');showToast('Verified at the table for 10 minutes.');renderPresence();return true}catch(e){showToast(e.message,true);return false}}
async function verifyAtTable(){try{const r=await fetch('/api/presence/request',{method:'POST',headers:authHeaders()});const d=await r.json();if(!r.ok)throw new Error(d.error||'Atlas could not show a code')}catch(e){showToast(e.message,true);return false}const code=await uiAsk({title:'Verify at the table',text:'The Atlas screen now shows a six-digit code for you. Enter it here, or scan the QR code on the Atlas screen with this phone.',ok:'Verify',input:{label:'Code from the Atlas screen',value:'',maxLength:7}});if(code===null)return false;return confirmPresence(code)}
async function ensureVerified(){const p=await presenceState();return (p&&p.verified)||verifyAtTable()}
async function presenceFetch(url,opts){let r=await fetch(url,opts);if(r.status===403){const d=await r.clone().json().catch(()=>({}));if(d.presenceRequired&&await verifyAtTable())r=await fetch(url,opts)}return r}
async function stopVerification(){try{await fetch('/api/presence/lock',{method:'POST',headers:authHeaders()});showToast('No longer verified at the table.')}catch(e){showToast(e.message,true)}renderPresence()}
async function renderPresence(){const el=document.getElementById('presenceStatus');if(!el)return;const p=await presenceState();el.textContent=!p?'':p.verified?`Verified at the table: ${Math.ceil(Number(p.remainingMs)/60000)} min left`:'Not verified at the table'}
async function presenceFromHash(){const m=/^#code=(\d{6})$/.exec(location.hash);if(!m)return;history.replaceState(null,'',location.pathname+location.search);if(!token){showToast('Sign in on this phone first, then enter the code.',true);return}await confirmPresence(m[1])}
async function renameSigil(id){const d=deviceById(id);if(!d)return;const next=await uiAsk({title:'Rename device',text:'You will be asked for the code the Atlas screen shows.',ok:'Save name',input:{label:`Custom name for ${d.defaultLabel||('Sigil '+(Number(id)+1))}`,value:d.customName||'',maxLength:32}});if(next===null)return;await saveSigilName(id,next.trim())}
...
async function forgetSigil(id){const d=deviceById(id);if(!d)return;if(!await uiAsk({title:'Forget this Sigil?',text:`Atlas forgets “${d.label}”. It must be paired again (Pair a Sigil on the Atlas screen, then the Sigil's Pair button) before anyone can use it.`,ok:'Forget Sigil',danger:true}))return;await forgetRequest(`module=${Number(id)}`)}
async function resetTableToLobby(){if(!await uiAsk({title:'Return table to lobby?',text:'A match in progress ends as a draw and every player is removed from the table. You will be asked for the code the Atlas screen shows.',ok:'Return to lobby',danger:true}))return;try{const r=await presenceFetch('/api/table/reset',{method:'POST',headers:authHeaders()});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not return the table to the lobby');resetTableStatus.textContent=d.message||'The table is back to an empty lobby.';showToast(resetTableStatus.textContent)}catch(e){resetTableStatus.textContent=e.message;showToast(e.message,true)}await refreshAll()}
async function forgetAllSigils(){if(!await uiAsk({title:'Forget all Sigils?',text:'Atlas forgets every paired Sigil. Each one must be paired again before anyone can use it.',ok:'Forget all',danger:true}))return;await forgetRequest('all=1')}
async function forgetRequest(query){try{const r=await fetch('/api/device/forget?'+query,{method:'POST',headers:authHeaders()});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not forget the Sigil');showToast(d.message||'Sigil forgotten.');await refreshAll()}catch(e){showToast(e.message,true)}}
async function factoryResetSigil(id){const d=deviceById(id);if(!d)return;if(!await uiAsk({title:'Factory reset this Sigil?',text:`“${d.label}” erases everything it has saved, including its pairing, and restarts. Atlas forgets it; pair it again to use it. You will be asked for the code the Atlas screen shows.`,ok:'Factory reset',danger:true}))return;await factoryResetRequest(`module=${Number(id)}`)}
async function factoryResetAtlas(){const typed=await uiAsk({title:'Factory reset Atlas?',text:'Atlas erases every profile, PIN, statistic it holds, Sigil pairing, the Wi-Fi password and all settings, then restarts as new. The microSD card is not erased. This cannot be undone. You will be asked for the code the Atlas screen shows.',ok:'Factory reset Atlas',danger:true,input:{label:'Type RESET to confirm',value:'',maxLength:5}});if(typed===null)return;if(String(typed).trim().toUpperCase()!=='RESET'){showToast('Not reset: type RESET to confirm.',true);return}await factoryResetRequest('atlas=1',true)}
async function factoryResetRequest(query,atlas){try{const r=await presenceFetch('/api/device/factory-reset?'+query,{method:'POST',headers:authHeaders()});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not factory reset');showToast(d.message||'Factory reset started.');if(atlas){resetTableStatus.textContent='Atlas is restarting as new. Rejoin its Wi-Fi with the default password, then set it up again.';return}await refreshAll()}catch(e){showToast(e.message,true)}}
function renderPairing(p){const sel=pairingWindowSelect;if(document.activeElement===sel||atlasHardwareDirty)return;const html=(p.choicesMs||[]).map(ms=>`<option value="${Number(ms)}">${Number(ms)/1000} seconds${Number(ms)===Number(p.sigilWindowMs)?' (default)':''}</option>`).join('');if(sel.innerHTML!==html)sel.innerHTML=html;sel.value=String(p.windowMs)}
function renderSpeaker(v){const sel=speakerVolumeSelect;if(document.activeElement===sel||atlasHardwareDirty)return;sel.value=String(v.volume)}
async function saveSpeakerVolume(){const sel=speakerVolumeSelect,label=sel.options[sel.selectedIndex].text;const r=await fetch('/api/speaker?volume='+Number(sel.value),{method:'POST',headers:authHeaders()});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not save the speaker volume');speakerVolumeStatus.textContent=`Atlas speaker volume: ${label}.`}
async function savePairingWindow(){const ms=Number(pairingWindowSelect.value);const r=await fetch('/api/pairing?windowMs='+ms,{method:'POST',headers:authHeaders()});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not save the pairing window');pairingWindowStatus.textContent=`Atlas pairing window: ${ms/1000} seconds.`}
async function saveAtlasHardware(){try{await savePairingWindow();await saveSpeakerVolume();atlasHardwareDirty=false;showToast('Atlas settings saved.')}catch(e){showToast(e.message,true);speakerVolumeStatus.textContent=e.message}}
async function saveSigilName(id,name){try{const r=await presenceFetch(`/api/device/name?module=${Number(id)}&name=${encodeURIComponent(name)}`,{method:'POST',headers:authHeaders()});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not save Sigil name');showToast(name?`Sigil renamed to ${d.label}.`:'Custom Sigil name cleared.');await refreshAll()}catch(e){showToast(e.message,true)}}
...
function renderDeviceSettings(d){const html=(d.devices||[]).map(x=>`<div class="setting-box"><div class="device-top"><div><div class="device-name">${esc(x.label)}</div><div class="mono">${esc(x.hardwareId)}</div></div><div class="actions"><button class="small" onclick="renameSigil(${x.id})">Rename</button>${x.customName?`<button class="small" onclick="clearSigilName(${x.id})">Clear name</button>`:''}<button class="small warn" onclick="forgetSigil(${x.id})" aria-label="Forget ${esc(x.label)}">Forget</button><button class="small warn" onclick="factoryResetSigil(${x.id})" aria-label="Factory reset ${esc(x.label)}">Factory reset</button></div></div></div>`).join('')||'<div class="empty">No Sigils discovered yet.</div>';if(deviceSettingsList.innerHTML!==html)deviceSettingsList.innerHTML=html}
...
async function setupAdmin(){try{if(!await ensureVerified())return;await accountPost('setup',{});adminSetup.hidden=true;await refreshAll();showTab('settings');showToast('Admin account established.')}catch(e){showToast(e.message,true)}}

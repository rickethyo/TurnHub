...
<div class="grid">
 ...
 <section class="card half"><div class="card-head"><div><h2 class="eyebrow">Paired Sigils</h2><p class="small">Unlock admin on the Atlas screen before renaming a device. Forget removes a Sigil's pairing; pair it again with Pair a Sigil on the Atlas screen and the Sigil's Pair button. Forget works in the lobby, for Sigils nobody is seated on. Holding a Sigil's own Pair button for 10 seconds makes the Sigil forget Atlas.</p></div></div><div id="atlasDevice"></div><div id="deviceSettingsList" class="stack" style="margin-top:10px;gap:10px"></div>
  <div class="actions" style="margin-top:10px"><button class="warn" onclick="forgetAllSigils()">Forget all Sigils</button><button class="warn" onclick="resetTableToLobby()">Return table to lobby</button></div>
  <p class="hint">Return table to lobby ends a match in progress as a draw (every player gets a draw in their statistics) and clears the table, even if nobody can reach their Sigil. Unlock admin on the Atlas screen first.</p>
...
function renderDeviceSettings(d){const html=(d.devices||[]).map(x=>`<div class="setting-box"><div class="device-top"><div><div class="device-name">${esc(x.label)}</div><div class="mono">${esc(x.hardwareId)}</div></div><div class="actions"><button class="small" onclick="renameSigil(${x.id})">Rename</button>${x.customName?`<button class="small" onclick="clearSigilName(${x.id})">Clear name</button>`:''}<button class="small warn" onclick="forgetSigil(${x.id})" aria-label="Forget ${esc(x.label)}">Forget</button></div></div></div>`).join('')||'<div class="empty">No Sigils discovered yet.</div>';if(deviceSettingsList.innerHTML!==html)deviceSettingsList.innerHTML=html}

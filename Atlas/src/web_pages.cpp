...
<p class="small">Off: no countdown; a gentle cue appears after 5 minutes. With a timer, Atlas warns 10 seconds before time runs out. Running out of time never passes the turn.</p>
...
function renderGameLife(){
 ...
 gameProfileSummary.textContent=settings?`${labels[settings.gameProfile]||settings.gameProfile} · Starting life ${settings.startingLife} · Turn timer ${timerLabel(settings.turnTimerMs||0)}`:'';
 renderLifeCards();
...
// Atlas decides the phase; the text carries the meaning, the badge only reinforces it.

...
from player_profiles import PlayerProfileStore
...
class TurnHub:
    def __init__(self) -> None:
        ...
        self.profiles = PlayerProfileStore(
            self.persistence.data_directory,
            venue_mode=__import__("os").environ.get("TURNHUB_VENUE_MODE", "").strip().lower() in ("1", "true", "yes", "venue"),
        )
        self._browser_feedback_seq = 0
        self._browser_feedback = None
    ...
    def _pass_running_turn(
        ...
        with self._turn_advance_lock:
            ...
            self.emit_browser_feedback("turn_pass")
            self.persistence.mark_dirty()
            return True
    ...
    def emit_browser_feedback(self, event: str, target_player: int | None = None) -> None:
        """Publish a lightweight event for virtual Sigil audio/haptics."""
        self._browser_feedback_seq += 1
        self._browser_feedback = {
            "seq": self._browser_feedback_seq,
            "event": str(event),
    ...
    def on_web_nudge(
        ...
        self.emit_browser_feedback("nudge_table" if table else "nudge", None if table else target_player)
    ...
    def save_game_log(self) -> None:
        ...
        # Associate the completed game with any persistent profiles that took
        # part. Guests remain intentionally anonymous across games.
        try:
            for player in self.game.players:
                profile_id = self.web_control.profile_id_for_seat(player.seat_key)
                if not profile_id:
                    continue
                stat = self.game.stats.get(player.player_number)
                self.profiles.append_history(profile_id, {
                    "finished_at_unix": time.time(),
                    "game_profile": self.game.game_profile,
                    "player_number": player.player_number,
                    "won": self.game.winner_player == player.player_number,
                    "turns_completed": stat.turns_completed if stat else 0,
                    "turn_seconds": stat.total_turn_seconds if stat else 0.0,
                })
        except Exception as exc:
            print(f"[PROFILE] Could not update game history: {exc}")

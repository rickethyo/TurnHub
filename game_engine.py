...
class GameEngine:
    ...
    def start(self, players: list[int], starter: int, warning_ms: int) -> None:
        self.players = list(players)
        self.active_index = self.players.index(starter)
        ...
        self.stats = {module: PlayerStats() for module in self.players}
    ...
    def current_turn_elapsed(self, now: float | None = None) -> float:
        if now is None:
            now = time.monotonic()

        end = now
        if self.state == STATE_PAUSED and self.pause_started_at is not None:
            end = self.pause_started_at
        elif self.state == STATE_GAME_OVER and self.game_ended_at is not None:
            end = self.game_ended_at

        return max(0.0, end - self.turn_started_at)

    def game_elapsed(self, now: float | None = None) -> float:
        if now is None:
            now = time.monotonic()

        end = self.game_ended_at if self.game_ended_at is not None else now
        return max(0.0, end - self.game_started_at - self.total_paused_seconds)

    def warning_phase(self, now: float | None = None) -> str:
        elapsed_ms = self.current_turn_elapsed(now) * 1000.0

        if elapsed_ms >= self.current_warning_ms:
            return "WARNING"

        if elapsed_ms >= self.current_warning_ms * WARNING_CAUTION_FRACTION:
            return "CAUTION"

        return "NORMAL"

    def pass_turn(self, module: int, next_warning_ms: int) -> bool:
    ...
    def pause(self, armed_by: int | None = None) -> bool:
        if self.state != STATE_RUNNING:
            return False
    ...
    def resume(self) -> bool:
        if self.state != STATE_PAUSED or self.pause_started_at is None:
            return False
    ...
    def declare_winner(self, module: int) -> bool:
    ...
    def freeze_for_hardware_loss(self) -> dict:
        """Capture enough state to restore timing after a hardware outage."""
        return {
    ...
    def restore_after_hardware_loss(self, outage: dict) -> None:
        now = time.monotonic()
        downtime = max(0.0, now - outage["disconnect_at"])
        self.total_hardware_downtime += downtime

        # Exclude hardware downtime from all game timing.
        ...
        # Reconnection always requires a deliberate long-press to continue.
